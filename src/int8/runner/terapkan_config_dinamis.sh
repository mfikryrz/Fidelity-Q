...
#
# SEBAB
#   repo_terpatch/ di src/int8/ adalah SALINAN DOKUMENTASI, diambil dari
#   mesin yang pernah jalan -- bukan sesuatu yang otomatis disalin balik ke
#   mesin baru. Beda dari memory_pool.py (yang punya patch_int8_memory_pool.sh
#   sendiri), _config.py dan fidelity_utils.py belum punya mekanisme serupa.
#
#   Topologi nyata di instance (dikonfirmasi dari jalankan_fp16.sh dan struktur
#   eksperimen yang pernah berjalan):
...
#                                         skrip fase 01-19 lain yang REPO-nya
...
#
#   graph.py dan inject_ops.py TIDAK disentuh skrip ini -- keduanya belum
#   diubah di repo_terpatch/ (lihat README.md bagian "Daftar operasi per
#   decoder"), jadi belum ada yang perlu diterapkan ulang untuk berkas itu.
#
...
# export_llama_int8.py: dipakai bootstrap_vast.sh tahap 50 (export SmoothQuant
# fake-INT8). Versi di sini menambah dukungan MODEL_ID berupa path lokal
# (21 Sep 2026) -- lihat komentar "DIPATCH" di repo_terpatch/export_llama_int8.py.
targets_export_llama_int8_py=(
  "$CODE/repo/FIdelity-ONNX-master/export_llama_int8.py"
)
...
# TIDAK ikut menggagalkan skrip kalau tidak ada -- export_llama_int8.py cuma
# dipakai kalau ONNX INT8 memang di-export lokal (jalur satu-satunya untuk
# INT8 saat ini, tapi tetap dibuat non-fatal untuk konsistensi dgn fp16).
terapkan "export_llama_int8.py" "${targets_export_llama_int8_py[@]}" \
  || echo "[info] export_llama_int8.py tidak disinkronkan -- periksa manual kalau memang dibutuhkan"
terapkan "modeling_llama.py" "${targets_modeling_llama_py[@]}" \
  || echo "[info] modeling_llama.py tidak disinkronkan -- periksa manual kalau memang dibutuhkan"
