...
# DITAMBAHKAN 21 Sep 2026 untuk dukungan GQA (Grouped-Query Attention).
# Implementasi persis sama dengan transformers.models.llama.modeling_llama.repeat_kv
# upstream -- sengaja expand+reshape, BUKAN torch.repeat_interleave, karena
# repeat_interleave punya riwayat trace ONNX yang tidak konsisten di beberapa
# opset/versi torch, sementara expand+reshape sudah terbukti export bersih.
#
# Untuk model MHA (num_key_value_heads == num_attention_heads, n_rep == 1):
# fungsi ini kembali lewat baris "if n_rep == 1: return hidden_states" TANPA
# menghasilkan satu node ONNX pun -- n_rep adalah int Python statis (dihitung
# sekali di __init__ dari config), jadi trace torch.onnx.export menghilangkan
# cabang ini sepenuhnya. Artinya graf Llama-2-7B (MHA) yang sudah tervalidasi
# 55.245 baris TIDAK berubah topologinya sama sekali oleh perubahan ini.
def repeat_kv(hidden_states: torch.Tensor, n_rep: int) -> torch.Tensor:
    """
    Setara torch.repeat_interleave(x, dim=1, repeats=n_rep). hidden_states
    berubah dari (batch, num_key_value_heads, seqlen, head_dim) menjadi
...
class LlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        # DITAMBAHKAN 21 Sep 2026: dukungan GQA. num_key_value_heads jatuh ke
        # num_heads kalau config tidak punya field itu sama sekali (model MHA
        # lama, atau tepatnya perilaku asli sebelum patch ini) -- num_key_value_groups
        # jadi 1 dan repeat_kv() di bawah adalah no-op murni untuk kasus itu.
        self.num_key_value_heads = getattr(config, "num_key_value_heads", self.num_heads)
        if self.num_heads % self.num_key_value_heads != 0:
            raise ValueError(
                f"num_attention_heads ({self.num_heads}) harus kelipatan bulat dari "
                f"num_key_value_heads ({self.num_key_value_heads}) untuk GQA yang valid."
            )
    ...
    def forward(
        ...
        # DIUBAH 21 Sep 2026: key/value_states memakai num_key_value_heads
        # (bisa < num_heads untuk GQA), bukan num_heads seperti query_states.
        ...
        # [bsz, nh, t, hd]  (key/value: nh = num_key_value_heads di sini)
        ...
        # Cache disimpan SEBELUM repeat_kv (num_key_value_heads, bukan num_heads)
        # -- konvensi sama seperti implementasi GQA upstream HF: KV cache tetap
        # hemat memori, perbesar ke num_heads cuma sesaat sebelum QK^T/attn*V.
        past_key_value = (key_states, value_states) if use_cache else None

        # DITAMBAHKAN 21 Sep 2026: perbesar K/V dari num_key_value_heads ke
        # num_heads sebelum QK^T. Untuk MHA (num_key_value_groups == 1) ini
        # no-op sempurna -- lihat komentar di definisi repeat_kv() di atas.
