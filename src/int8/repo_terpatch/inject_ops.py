...
def _onnx_pick_aligned_block_nodes(
    dim_tensor: str,
    block_len_tensor: str,
    suffix: str,
    tag: str = "abl",
) -> Tuple[List, str, str]:
    """Build ONNX nodes that pick a random partition-aligned block on one axis.

    Partitions ``dim_tensor`` into fixed blocks of ``block_len_tensor`` elements
    (the last block may be shorter).  Returns ``(nodes, start_idx, valid_len)``
    tensor names for the half-open window [start_idx, start_idx + valid_len).
    """
    p = tag + suffix
    nodes = []

    nodes.append(helper.make_node(
        "Constant", [], [f"abl_one{p}"],
        value=helper.make_tensor(f"abl_one_t{p}", TensorProto.INT64, [], [1]),
    ))
    nodes.append(helper.make_node(
        "Sub", [block_len_tensor, f"abl_one{p}"], [f"abl_bm1{p}"],
    ))
    nodes.append(helper.make_node(
        "Add", [dim_tensor, f"abl_bm1{p}"], [f"abl_dplus{p}"],
    ))
    nodes.append(helper.make_node(
        "Div", [f"abl_dplus{p}", block_len_tensor], [f"abl_nbraw{p}"],
    ))
    nodes.append(helper.make_node(
        "Max", [f"abl_nbraw{p}", f"abl_one{p}"], [f"abl_nblocks{p}"],
    ))
    nodes.append(helper.make_node(
        "Cast", [f"abl_nblocks{p}"], [f"abl_nbf{p}"], to=TensorProto.FLOAT,
    ))
    nodes.append(helper.make_node(
        "RandomUniform", [], [f"abl_rand{p}"],
        dtype=TensorProto.FLOAT, high=1.0, low=0.0, shape=[1],
    ))
    nodes.append(helper.make_node(
        "Mul", [f"abl_rand{p}", f"abl_nbf{p}"], [f"abl_rscaled{p}"],
    ))
    nodes.append(helper.make_node(
        "Floor", [f"abl_rscaled{p}"], [f"abl_rfloor{p}"],
    ))
    nodes.append(helper.make_node(
        "Cast", [f"abl_rfloor{p}"], [f"abl_bid{p}"], to=TensorProto.INT64,
    ))
    nodes.append(helper.make_node(
        "Mul", [f"abl_bid{p}", block_len_tensor], [f"abl_start{p}"],
    ))
    nodes.append(helper.make_node(
        "Sub", [dim_tensor, f"abl_start{p}"], [f"abl_remain{p}"],
    ))
    nodes.append(helper.make_node(
        "Min", [block_len_tensor, f"abl_remain{p}"], [f"abl_valid{p}"],
    ))

    return nodes, f"abl_start{p}", f"abl_valid{p}"


def _onnx_pick_random_subwindow_nodes(
    block_start_tensor: str,
    block_valid_tensor: str,
    suffix: str,
    tag: str = "rsw",
) -> Tuple[List, str, str]:
    """Pick a random contiguous sub-window within an already-selected block.

    Returns ``(nodes, win_start, win_len)`` for the half-open window
    [win_start, win_start + win_len) where ``1 <= win_len <= block_valid``.
    """
    p = tag + suffix
    nodes = []

    nodes.append(helper.make_node(
        "Constant", [], [f"rsw_one{p}"],
        value=helper.make_tensor(f"rsw_one_t{p}", TensorProto.INT64, [], [1]),
    ))
    nodes.append(helper.make_node(
        "Cast", [block_valid_tensor], [f"rsw_vf{p}"], to=TensorProto.FLOAT,
    ))
    nodes.append(helper.make_node(
        "RandomUniform", [], [f"rsw_cr{p}"],
        dtype=TensorProto.FLOAT, high=1.0, low=0.0, shape=[1],
    ))
    nodes.append(helper.make_node(
        "Mul", [f"rsw_cr{p}", f"rsw_vf{p}"], [f"rsw_cscaled{p}"],
    ))
    nodes.append(helper.make_node(
        "Floor", [f"rsw_cscaled{p}"], [f"rsw_cfloor{p}"],
    ))
    nodes.append(helper.make_node(
        "Cast", [f"rsw_cfloor{p}"], [f"rsw_craw{p}"], to=TensorProto.INT64,
    ))
    nodes.append(helper.make_node(
        "Add", [f"rsw_craw{p}", f"rsw_one{p}"], [f"rsw_count{p}"],
    ))

    nodes.append(helper.make_node(
        "Sub", [block_valid_tensor, f"rsw_count{p}"], [f"rsw_maxoff{p}"],
    ))
    nodes.append(helper.make_node(
        "Add", [f"rsw_maxoff{p}", f"rsw_one{p}"], [f"rsw_span{p}"],
    ))
    nodes.append(helper.make_node(
        "Cast", [f"rsw_span{p}"], [f"rsw_spanf{p}"], to=TensorProto.FLOAT,
    ))
    nodes.append(helper.make_node(
        "RandomUniform", [], [f"rsw_or{p}"],
        dtype=TensorProto.FLOAT, high=1.0, low=0.0, shape=[1],
    ))
    nodes.append(helper.make_node(
        "Mul", [f"rsw_or{p}", f"rsw_spanf{p}"], [f"rsw_oscaled{p}"],
    ))
    nodes.append(helper.make_node(
        "Floor", [f"rsw_oscaled{p}"], [f"rsw_ofloor{p}"],
    ))
    nodes.append(helper.make_node(
        "Cast", [f"rsw_ofloor{p}"], [f"rsw_offset{p}"], to=TensorProto.INT64,
    ))

    nodes.append(helper.make_node(
        "Add", [block_start_tensor, f"rsw_offset{p}"], [f"rsw_start{p}"],
    ))

    return nodes, f"rsw_start{p}", f"rsw_count{p}"
...
def create_weight16_mask(matmul_output="y", masked_output="y_masked", block_length=16, fp16=True):
    nodes = []
    suffix = "_mask"
    ...
    # 8–10. Pick a random partition-aligned block on the sequence axis
    abl_nodes, abl_start, abl_valid = _onnx_pick_aligned_block_nodes(
        "seq_len_tensor" + suffix, "block_len" + suffix, suffix, tag="w16",
    )
    sub_nodes, win_start, win_len = _onnx_pick_random_subwindow_nodes(
        abl_start, abl_valid, suffix, tag="w16",
    )
    nodes.extend(abl_nodes)
    nodes.extend(sub_nodes)
    nodes.append(helper.make_node(
        "Identity", [win_start], ["start_idx" + suffix],
    ))
    nodes.append(helper.make_node(
        "Identity", [win_len], ["valid_block" + suffix],
    ))
...
def create_input16_mask(matmul_output="y", masked_output="y_masked", block_length=16, fp16=False):
    ...
    # 6–7. Pick a random partition-aligned block on the feature axis
    abl_nodes, abl_start, abl_valid = _onnx_pick_aligned_block_nodes(
        "feature_len" + suffix, "block_len" + suffix, suffix, tag="i16",
    )
    sub_nodes, win_start, win_len = _onnx_pick_random_subwindow_nodes(
        abl_start, abl_valid, suffix, tag="i16",
    )
    nodes.extend(abl_nodes)
    nodes.extend(sub_nodes)
    nodes.append(helper.make_node(
    ...
    nodes.append(helper.make_node(
        "Add",
        inputs=["start_idx" + suffix, win_len],
        outputs=["end_idx" + suffix]
    ))
...
def create_fc_input16_mask(fc_output="y", masked_output="y_masked", block_length=16, fp16=True):
    ...
    abl_nodes, abl_start, abl_valid = _onnx_pick_aligned_block_nodes(
        "Fdim"+suffix, "blk"+suffix, suffix, tag="fci",
    )
    sub_nodes, win_start, win_len = _onnx_pick_random_subwindow_nodes(
        abl_start, abl_valid, suffix, tag="fci",
    )
    nodes.extend(abl_nodes)
    nodes.extend(sub_nodes)
    nodes.append(helper.make_node("Identity", [win_start], ["sidx"+suffix]))
    nodes.append(helper.make_node("Identity", [win_len], ["vblock"+suffix]))
...
def create_fc_weight16_mask(fc_output="y", masked_output="y_masked", block_length=16, fp16=True):
    ...
    nodes.append(helper.make_node("Constant", [], ["blk"+suffix],
        value=helper.make_tensor("blk_t"+suffix, TensorProto.INT64, [], [block_length])))
    abl_nodes, abl_start, abl_valid = _onnx_pick_aligned_block_nodes(
        "Rdim"+suffix, "blk"+suffix, suffix, tag="fcw",
    )
    sub_nodes, win_start, win_len = _onnx_pick_random_subwindow_nodes(
        abl_start, abl_valid, suffix, tag="fcw",
    )
    nodes.extend(abl_nodes)
    nodes.extend(sub_nodes)
    nodes.append(helper.make_node("Identity", [win_start], ["sidx"+suffix]))
...
def create_conv_input16_mask(conv_output="y", masked_output="y_masked", block_length=16, fp16=True):
    ...
    nodes.append(helper.make_node("Constant", [], ["blk"+suffix],
        value=helper.make_tensor("blk_t"+suffix, TensorProto.INT64, [], [block_length])))
    abl_nodes, abl_start, abl_valid = _onnx_pick_aligned_block_nodes(
        "Cdim"+suffix, "blk"+suffix, suffix, tag="cci",
    )
    sub_nodes, win_start, win_len = _onnx_pick_random_subwindow_nodes(
        abl_start, abl_valid, suffix, tag="cci",
    )
    nodes.extend(abl_nodes)
    nodes.extend(sub_nodes)
    nodes.append(helper.make_node("Identity", [win_start], ["sidx"+suffix]))
...
def create_conv_weight16_mask(conv_output="y", masked_output="y_masked", block_length=16, fp16=True):
    ...
    abl_nodes, abl_start, abl_valid = _onnx_pick_aligned_block_nodes(
        "Wdim"+suffix, "blk"+suffix, suffix, tag="ccw",
    )
    sub_nodes, win_start, win_len = _onnx_pick_random_subwindow_nodes(
        abl_start, abl_valid, suffix, tag="ccw",
    )
    nodes.extend(abl_nodes)
    nodes.extend(sub_nodes)
    nodes.append(helper.make_node("Identity", [win_start], ["sidx"+suffix]))
